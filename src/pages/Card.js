...
const Card = () => {
  const { onFavorites, onAddToCart, isCardAdded, isFavAdded } = useContext(AppContext);
  const { id } = useParams();
    ...
    try {
      (async () => {
        const { data } = await axios.get(
          'https://66b9c6c4fa763ff550f97640.mockapi.io/products/' + id,
        );
        setProduct(data);
      })();
    } catch (error) {
            ...
            <div className="content__btns">
              <button onClick={addToCart}>
                <svg
                  ...
                  height="20"
                  viewBox="0 0 20 20"
                  fill={isCardAdded(product.id) ? '#2b7b39' : 'none'}
              ...
              <button onClick={addLike}>
                <svg
                  ...
                  height="14"
                  viewBox="0 0 16 14"
                  fill={isFavAdded(product.id) ? '#f9487a' : 'none'}
                  xmlns="http:www.w3.org/2000/svg">
                  <path
